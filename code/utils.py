...
def get_bge_embeddings( sentences ):
    ...
    if _bge_model is None:
        ...
        bge_path = os.path.join(current_path, "model/BAAI/")
...
def get_luotuo_model():
    ...
    luotuo_path = '/group/30106/warrensun/kg/new_data/llm/luotuo-bert-medium'
    model_args = Namespace(do_mlm=None, pooler_type="cls", temp=0.05, mlp_only_train=False,
                           init_embeddings_model=None)

    luotuo_model = AutoModel.from_pretrained(luotuo_path, trust_remote_code=True, model_args=model_args).to(device)

    # luotuo_model = download_models()
...
def luotuo_embedding(model, texts):
    ...
    luotuo_path = '/group/30106/warrensun/kg/new_data/llm/luotuo-bert-medium'
    tokenizer = AutoTokenizer.from_pretrained(luotuo_path)
